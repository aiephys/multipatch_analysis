...
class FirstPulseFitTableGroup(TableGroup):
    ...
    schemas = {
        'individual_first_pulse_fit': [
            "Best parameters fit to individual first pulses.  ",
...
def update_fit(limit=None, parallel=True, workers=6, raise_exceptions=False, session=None):
    """Update individual first pulse fit table for all experiments
    """
    ...
    s = db.Session()
    # get all pairs that are in pair table but are not in FirstPulseFeatures table
...
#    jobs = [(record.acq_timestamp, index, len(experiments)) for index, record in enumerate(experiments)]
    pairs = [record[0] for record in enumerate(records)]
