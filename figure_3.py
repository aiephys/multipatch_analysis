...
from manuscript_figures import cache_response, get_amplitude, response_filter, feature_anova, write_cache, trace_plot
...
synapse_plot = [] #(grid[0, 0], grid[1, 0], grid[2, 0], grid[3, 0], grid[4, 0])
...
for c in range(len(connection_types)):
    ...
    color = (c, len(connection_types)*2)
