        ...
        if headstages is not None:
            ...
            if no_tubes is False:
                ...
                for cell_id, cell in expt.cells.items():
                    tube_id = patchseq_tubes.get(cell.ext_id)
                    if tube_id is None:
                        continue

                    results = {
                        'tube_id' = tube_id
                        'nucleus' = nucleus[cell.ext_id],
                        'patchseq_hash' = None,
                    }
                    ...
                    results['patchseq_hash'] = patch_seq_hash

                    for result_name, col_name in col_names.items():
                        data = getattr(patchseq_results, result_name)
                        if data is not None:
                            if col_name == 'meta':
...
def ready_jobs(self):
    ...
    for expt_id, (expt_mtime, success) in expts.items():
        ...
        for cell_ext_id, cell in expt.cells.items():
            tube_id = patchseq_tubes.get(cell_ext_id)
            if tube_id is None:
                continue
...
def get_amp_results():
    ...
    _, local_copy = tempfile.mkstemp(prefix='amplification_results', suffix='.xlsx')
    ...
    amp_dict = amp_results_reduced.to_dict('index')
...
def get_mapping_results():
    ...
    _, mouse_local = tempfile.mkstemp(prefix='mapping_mouse', suffix='.csv')
    shutil.copy(mouse_path, mouse_local)
    mouse_results = pd.read_csv(mouse_local, header=0, index_col=False, dtype=object)
    _, human_local = tempfile.mkstemp(prefix='mapping_human', suffix='.csv')
    ...
    mapping_dict = mapping_results.to_dict('index')
